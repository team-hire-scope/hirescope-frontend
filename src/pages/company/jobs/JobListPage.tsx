		...
		<section className='w-full space-y-6'>
			<div className='flex items-center justify-between'>
				<div>
					...
					<p className='mt-1 text-sm text-black'>현재 진행 중인 공고와 지원 현황을 확인하세요.</p>
				</div>
				<Button>공고 추가</Button>
			</div>
			<div className='grid w-full grid-cols-2 gap-4'>
				{jobs.map(job => (
					<article key={job.id} className='rounded-xl border border-hs-cream bg-white p-5 shadow-sm'>
						...
						<p className='text-sm text-black'>지원자 {job.applicants}명</p>
					</article>
				))}
			</div>
		</section>
